...
def init(date_now):
    if not os.path.exists(f'csv-original\\sh000300_{date_now}.csv'):
...
def join_300(ticker):
    ...
    df_300 = pd.read_csv(f'csv-original\\sh000300_{date_now}_pct_300.csv')
    ...
    df.to_csv(f'csv-original\\{ticker}_{date_now}.csv')
